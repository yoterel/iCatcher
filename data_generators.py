...
class ImageDataGenerator(object):
    ...
    def flow_from_directory(self, directory,
                            ...
                            classes=None, class_mode='categorical', sequence_number=0,
    ...
    def fit(self, x,
       ...
       """
        ...
        if self.featurewise_center:
            self.mean = np.mean(x, axis=(0, self.row_axis, self.col_axis), dtype=np.float64)
        ...
        if self.featurewise_std_normalization:
            self.std = np.std(x, axis=(0, self.row_axis, self.col_axis), dtype=np.float64)
            broadcast_shape = [1, 1, 1]
            broadcast_shape[self.channel_axis - 1] = x.shape[self.channel_axis]
...
def _iter_valid_files(directory, sequence_number, white_list_formats, follow_links):
    ...
    seq_string = 'x{:02d}x'.format(sequence_number)
    pattern = '*{}*.png'.format(seq_string)
    for root, _, files in _recursive_list(directory):
        for fname in sorted(fnmatch.filter(files, pattern)):
            yield root, fname


def _list_valid_filenames_in_directory(directory, seq_number, white_list_formats, split,
    ...
    if split:
        num_files = len(list(
            _iter_valid_files(directory, seq_number, white_list_formats, follow_links)))
        start, stop = int(split[0] * num_files), int(split[1] * num_files)
        valid_files = list(
            _iter_valid_files(
                directory, seq_number, white_list_formats, follow_links))[start: stop]
    else:
        valid_files = _iter_valid_files(
            directory, seq_number, white_list_formats, follow_links)
    if df:
        filenames = []
...
class DirectoryIterator(Iterator):
    ...
    def __init__(self, directory, image_data_generator,
        ...
        for dirpath in (os.path.join(directory, subdir) for subdir in classes):
            results.append(
                pool.apply_async(_list_valid_filenames_in_directory,
                                 (dirpath, sequence_number, white_list_formats, self.split,
        ...
        logging.info('Found %d images belonging to %d classes.' %
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        for i, j in enumerate(index_array):
            ...
            if hasattr(img, 'close'):
                img.close()
            params = self.image_data_generator.get_random_transform(x.shape, j + i)
            x = self.image_data_generator.apply_transform(x, params)
            x = self.image_data_generator.standardize(x)
            # if self.seed is not None:
            #     np.random.seed(self.seed)
            if self.image_data_generator.mask_image:
                black = np.random.randint(5)
                if black == self.sequence_number and self.sequence_number != 2:
                    x[:] = 0
            batch_x[i] = x
...
class DataFrameIterator(Iterator):
    ...
    def __init__(self, dataframe, directory, image_data_generator,
        ...
        if self.num_classes > 0:
            ...
            logging.info('Found %d images.' % self.samples)
...
class FiveImageGenerator:
    def __init__(self, generator, source_dir, image_size, batch_size, shuffle, reverse_aug, seed):
        self.gen1 = generator.flow_from_directory(
            source_dir,  # Source directory for the training images
            ...
            shuffle=shuffle,
            seed=seed)
        self.gen2 = generator.flow_from_directory(
            source_dir,  # Source directory for the training images
            target_size=(image_size, image_size),
            batch_size=batch_size,
            sequence_number=1,
            ...
            shuffle=shuffle,
            seed=seed)
        self.gen3 = generator.flow_from_directory(
            source_dir,  # Source directory for the training images
            target_size=(image_size, image_size),
            batch_size=batch_size,
            sequence_number=2,
            ...
            shuffle=shuffle,
            seed=seed)
        self.gen4 = generator.flow_from_directory(
            source_dir,  # Source directory for the training images
            target_size=(image_size, image_size),
            batch_size=batch_size,
            sequence_number=3,
            ...
            shuffle=shuffle,
            seed=seed)
        self.gen5 = generator.flow_from_directory(
            source_dir,  # Source directory for the training images
            target_size=(image_size, image_size),
            batch_size=batch_size,
            sequence_number=4,
            ...
            shuffle=shuffle,
            seed=seed)
        self.samples = self.gen1.samples
        self.batch_size = self.gen1.batch_size
    ...
    def generate(self):
        while True:
            ...
            y = X3[1]
            if self.reverse_aug:
                away_class_binary_reversed = np.nonzero(X3[1] == 1)
                away_class_binary = 1-away_class_binary_reversed[1]
                rand_vect = np.random.rand(away_class_binary.shape[0])
                rand_vect_binary = rand_vect <= 0.5
                away_class_indices = np.nonzero(np.logical_and(away_class_binary, rand_vect_binary))
                temp1 = X1[0][away_class_indices]
                temp2 = X2[0][away_class_indices]
                X1[0][away_class_indices] = X5[0][away_class_indices]
                X2[0][away_class_indices] = X4[0][away_class_indices]
                X5[0][away_class_indices] = temp1
                X4[0][away_class_indices] = temp2
            yield x, y